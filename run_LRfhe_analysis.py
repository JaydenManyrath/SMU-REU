...
facial recognition model, designed for drone applications.
...
def generate_svd_visualization(original_image, k_values, optimal_k, output_path):
    ...
    num_k = len(k_values)
    fig, axs = plt.subplots(2, (num_k + 2) // 2, figsize=(15, 6))
    axs = axs.flatten()
    ...
    for i, k in enumerate(k_values):
        ...
        if k == optimal_k:
            ...
            ax.set_title(f"k = {k} (Optimal)", color='red')
...
def main():
    ...
    # ... (This section remains unchanged, but now benefits from the trained model) ...
    ...
    plt.title('Accuracy vs. Data Payload Size Trade-off')
    fig.tight_layout()
